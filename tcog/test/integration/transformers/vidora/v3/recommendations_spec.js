...
describe('Vidora integration: recommendations V3', () => {
    beforeEach((done) => {
        capi = nock(`${conf.capiV3API}`)
                .log(console.log)
                .get(/v3\/articles\/.*?api_key=.*/)
                .times(10)
    ...
    describe('Vidora 200', () => {
        ...
        it('uses the first 10 articles recommended', (done) => {
            vidora = nock(`${conf.vidoraAPI}`)
                        .log(console.log)
                        .get(`/v1/users/${userId}/recommendations/?api_key=${conf.vidoraApiKey}`)
                        .reply(200, (uri, requestBody) => {
            ...
            .get('/123?t_product=HeraldSun&td_device=desktop&t_output=json')
            .end(function(err, res) {
                expect(capiQueries.length).to.equal(10);
                done(err);
            });
        });

        it('returns a maximum of 10 articles', (done) => {
            // Mock vidora call returns 15 articles
            const excessVidoraData = {
                items: [
                    {'id': '4f1cb432c42ee68b741df246dff74283'},
                    {'id': 'fe38f8bc6dc147bd5b02c3968e4bef68'},
                    {'id': 'e2f7dd83cdc417b1cc6586ddcee60942'},
